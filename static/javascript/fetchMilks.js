...
function fetchProducts(skip = 0, keyword = '') {
  let url = `http://localhost:8000/products/categories/15?skip=${skip}`;
  if (keyword) {
    url += `&search=${keyword}`;
...
function updateProductContainer(products) {
  ...
  products.forEach(product => {
    ...
    const randomImageIndex = Math.floor(Math.random() * 8) + 1; // Assuming 8 previous images
    productImage.src = `../img/milk${randomImageIndex}.jpg`;

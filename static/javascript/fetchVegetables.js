...
function fetchProducts(skip = 0, keyword = '') {
  let url = `http://localhost:8000/products/categories/10?skip=${skip}`;
  if (keyword) {
    url += `&search=${keyword}`;
...
function updateProductContainer(products) {
  ...
  products.forEach(product => {
    ...
    const randomImageIndex = Math.floor(Math.random() * 7) + 1; // Assuming 7 previous images
    productImage.src = `../img/vegetable${randomImageIndex}.jpg`;
